...
latency. The only difference is that with two connections the confidence
intervals for \cdn and \lt-double are exceptionally large for some of the
formats.
...
requests are sent for any of the caching setups.
...
\subsubsection{Transfer speed measurements}
The figures~\ref{fig:after_other_mbps}
and~\ref{fig:after_other_requests_per_second} show a couple of things again:
\begin{enumerate}
    \item
    ...
    \item
        When comparing received MB/s it seems that the order in which two
        formats are requested does not matter much. Complementary plots are
        very similar, for instance requesting \dash after \iss performs about as
        well as requesting \iss after \dash

    \item
        The \lt setups perform better0
\end{enumerate}
