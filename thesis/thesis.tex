...
\section{Results}
The results of the different measurements are shown seperatly for the three
different cache conditions. The transfer speed and latancy measurements are
...
but the important imformation that can be gathered from the plots is noted as
well.
...
\begin{enumerate}
    \item
        The \ipp and \lt-nocache perform exactly the same as do the \cdn type
        setups and the caching \lt setups do this as well.
    ...
    \item
        The amount of requets for \hds and \hls than the amount of requests for
        \dash and \iss is higher when looking at the \ipp and \lt type setups,
        but it is lower when looking at the \cdn type setups.
...
\subsubsection{Cache usage}
First, to reiterate, the bars represententing requesting content after itself
...
\begin{enumerate}
    ...
    \item In a some of the cases the cache usage of the \lt-single setup is a
        bit more than the cache usage when requesting the same format with a
        cold cache. These cases are \hds and \hls after \dash and \iss and the
        other way around. When requesting \dash after \iss or \hls after \hds or
        both the other way around the cache usage stays the same.
...
\begin{enumerate}
    ...
    \item
        With the \lt setups when requesting \dash after \iss, \iss after \dash or \hds after \hls no
        internal data is sent.

    \item With the \lt setups when requesting \hls after \hds only a very small
        amount of requests is sent internally.

    \item With the \lt setups when requesting \hds or \hls after \dash or \iss about 150 internal
        requests are sent. When \dash or \iss are requested after \hds or \hls
        about 180 internal requests are sent. The amount of data that is sent in
        all these cases is all roughly the same.

    \item
        In the cases mentioned in the previous item the amount of requests for
        the \lt setups is about half of that of the \cdn setup in the same
        cases. The amount of data sent internally with the \lt setups is a
        little more than a tenth of the data sent internally using the \cdn
        setup.
...
experiments described in the previous chapter. It also suggets opportunities for
future research subjects.

\section{Explanation of the results}
To explain the results, they are once again split into the different cache
